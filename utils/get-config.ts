...
export async function getVar<T>(params: Params<T>): Promise<T> {
  ...
  async function validatePrint<T>(value: string): Promise<T> {
    try {
      ...
      console.log(`${params.argName || params.envName}: ${params.secret ? formatSecret(v) : v}`);
      return v;
    } catch (error) {
      throw new Error(`Invalid value provided for ${argName} (${envName}): "${value}"\n ${error}`);
    }
  }
  ...
  if (argName) {
    const argValue = yargs(process.argv.slice(2))[argName] as string | undefined;
    if (argValue) {
